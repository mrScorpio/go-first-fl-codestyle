...
func defence(char_name, char_class string) string {
	if char_class == "warrior" {
		return fmt.Sprintf("%s блокировал %d урона.", char_name, 10+randNum(5, 10))
	}

	if char_class == "mage" {
		return fmt.Sprintf("%s блокировал %d урона.", char_name, 10+randNum(-2, 2))
	}

	if char_class == "healer" {
		return fmt.Sprintf("%s блокировал %d урона.", char_name, 10+randNum(2, 5))
	}
